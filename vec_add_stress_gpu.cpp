...
#include "kernel_vec_add.h"

using namespace std;


int vec_add_stress_gpu(cl_device_id device_id) {
    ...
    cl_program program = clCreateProgramWithSource(context, 1, (const char**)&kernel_vec_add, NULL, &ret);
    if (DEBUG) {
        if (ret == CL_SUCCESS) {
            if (DEBUG_SUCCESS) cout << "success" << endl;
        }
    ...
    //for(int i = 0; i < array_dimension; i++) cout << A[i] << "\t" << B[i] << "\t"  << C[i] << endl;
